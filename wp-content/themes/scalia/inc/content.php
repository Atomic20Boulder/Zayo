...
?>
	<div class="block-content">
		<div class="container">
			<h1 class="page-title"><?php _e('Scalia Multi-Purpose Theme', 'scalia') ?></h1>
			<div class="inner">
...
function scalia_contacts($top_area = false) {
	...
	if(scalia_get_option('contacts_address')) {
		$output .= '<div class="sc-contacts-item sc-contacts-address">'.esc_html(scalia_get_option('contacts_address')).'</div>';
	}
	if(scalia_get_option('contacts_phone')) {
		$output .= '<div class="sc-contacts-item sc-contacts-phone">'.(!$top_area ? __('Phone:', 'scalia') : '').' '.esc_html(scalia_get_option('contacts_phone')).'</div>';
	}
	if(scalia_get_option('contacts_fax')) {
		$output .= '<div class="sc-contacts-item sc-contacts-fax">'.(!$top_area ? __('Fax:', 'scalia') : '').' '.esc_html(scalia_get_option('contacts_fax')).'</div>';
	}
	...
	if($posts->have_posts()) {
		if ($params['blog_style'] == 'grid_carousel') {
			...
			<?php if($params['blog_pagination'] == 'more' && !$params['is_ajax'] && $posts->max_num_pages > $paged): ?>
				<div class="blog-load-more col-lg-12">
