...
class Scalia_Widget_Popular_Posts extends WP_Widget {
	function __construct() {
		...
		$this->WP_Widget('Custom_Popular_Posts', __('Custom Popular Posts', 'scalia'), $widget_ops);
	}
	...
	if (!empty($posts)) {
		...
		foreach ($posts as $post) {
			...
			if (!empty($image_thumb)) {
				$return_html .= '<div class="sc-pp-posts-image"><a href="' . get_permalink($post->ID) . '"><img src="' . $image_thumb[0] . '" alt=""/></a></div>';
			} else {
			...
			$return_html .= '<div class="sc-pp-posts-date">' . apply_filters('get_the_date', mysql2date(get_option('date_format'), $post->post_date), '') . '</div></div></li>';
		}
		$return_html .= '</ul>';
	}
	if ($echo) {
...
class Scalia_Widget_Recent_Posts extends WP_Widget {
	function Scalia_Widget_Recent_Posts() {
		...
		$this->WP_Widget('Custom_Recent_Posts', __('Custom Recent Posts', 'scalia'), $widget_ops);
	}
...
class Scalia_Widget_Flickr extends WP_Widget {
	function Scalia_Widget_Flickr() {
		...
		$this->WP_Widget('Custom_Flickr', __('Flickr', 'scalia'), $widget_ops);
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
...
class Scalia_Widget_Submenu extends WP_Widget {
	function __construct()
	{
		...
		$this->WP_Widget('Submenu', __('Submenu', 'scalia'), $widget_ops);
	}
...
class Scalia_Widget_Facebook extends WP_Widget {
	function __construct()
	{
		...
		$this->WP_Widget('Facebook', __('Facebook', 'scalia'), $widget_ops);
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
...
class Scalia_Widget_Teams extends WP_Widget {
	function __construct() {
		...
		$this->WP_Widget('Teams', __('Teams', 'scalia'), $widget_ops);
	}
