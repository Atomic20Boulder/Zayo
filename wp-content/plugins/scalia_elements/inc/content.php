...
function scalia_portfolio($params) {
	...
	if($params['pagination'] == 'more') {
		...
		$portfolio_loop = new WP_Query(array(
			...
			'scalia_portfolios' => $params['portfolio'],
			'post_status' => 'publish',
			'orderby' => 'menu_order ID',
			'order' => 'ASC',
...
?>
	<?php if($item_data['link']) : ?>
		<a href="<?php echo esc_url($item_data['link']); ?>" target="<?php echo esc_attr($item_data['link_target']); ?>" class="slide">
	<?php else : ?>
