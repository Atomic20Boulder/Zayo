...
add_action('init', 'scalia_portfolio_item_post_type_init');
...
?>
	<div id="portfolio_item_type_<?php echo $types_index; ?>_wrapper">
	...
	<div id="portfolio_item_link_target_<?php echo $types_index; ?>_wrapper">
	...
	<div id="portfolio_item_link_<?php echo $types_index; ?>_wrapper">
...
<p class="meta-options">
	...
	<div id="portfolio_item_type_template" style="display: none;">
...
function scalia_get_sanitize_pf_item_data($post_id = 0, $item_data = array()) {
	...
	foreach ($portfolio_item_data['types'] as $k => $v) {
		$portfolio_item_data['types'][$k]['link_target'] = scalia_check_array_value(array('_blank', '_self'), $v['link_target'], '_self');
		$portfolio_type_options = array_keys($PORTFOLIO_TYPE_OPTIONS);
		$portfolio_item_data['types'][$k]['type'] = scalia_check_array_value($portfolio_type_options, $v['type'], 'self-link');
		if (!in_array($portfolio_item_data['types'][$k]['type'], array('youtube', 'vimeo')))
			$portfolio_item_data['types'][$k]['link'] = esc_url($v['link']);
	}
	$portfolio_item_data['types'] = array_slice($portfolio_item_data['types'], 0, 4);
